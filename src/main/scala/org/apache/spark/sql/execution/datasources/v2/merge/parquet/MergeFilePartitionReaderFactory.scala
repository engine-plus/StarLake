...
import org.apache.spark.sql.execution.datasources.v2.merge.parquet.batch.{MergeParquetFilePartitionByBatchFile, MergeParquetSingletonFilePartitionByBatchFile}
...
abstract class MergeFilePartitionReaderFactory extends PartitionReaderFactory {
  override def createReader(partition: InputPartition): PartitionReader[InternalRow] = {
    ...
    val iter = filePartition.files.toIterator.map { file =>
      assert(file.isInstanceOf[MergePartitionedFile])
      file -> buildColumnarReader(file)
    }.toSeq
