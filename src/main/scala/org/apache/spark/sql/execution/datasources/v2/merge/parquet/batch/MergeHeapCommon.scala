...
trait MergeHeapCommon {
  ...
  val versionKeyInfoMap: Map[Long, Array[(Int, DataType)]]
  ...
  val comparatorT = new Comparator[heapType] {
    override def compare(x: (Long, BufferedIterator[(InternalRow, Int)]),
      ...
      for (i <- versionKeyInfoMap(x._1).indices) {
        ...
        val ordinalX = versionKeyInfoMap(x._1)(i)._1
        val ordinalY = versionKeyInfoMap(y._1)(i)._1
        val comparV =
          versionKeyInfoMap(x._1)(i)._2 match {
            ...
            case _ => throw new RuntimeException("Unsupported data type for merge,type is " + versionKeyInfoMap(x._1)(i)._2.getClass.getTypeName)
          }
        if (comparV != 0) {
          return comparV
        }
...
class MergeOptimizeHeap(versionKey: Map[Long, Array[(Int, DataType)]]) extends MergeHeapCommon {
...
class MergePriorityQ(versionKey: Map[Long, Array[(Int, DataType)]]) extends MergeHeapCommon {
