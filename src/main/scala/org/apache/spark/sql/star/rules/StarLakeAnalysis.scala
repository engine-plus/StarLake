...
import org.apache.spark.sql.star.{StarLakeTable, StarLakeTableRelationV2}
...
/**
  ...
  * INSERT INTO.
  */
case class StarLakeAnalysis(session: SparkSession, sqlConf: SQLConf)
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsDown {
    ...
    case a@AppendData(DataSourceV2Relation(d: StarLakeTableV2, _, _, _, _), query, _, false)
    ...
    DataSourceV2Relation(d: StarLakeTableV2, _, _, _, _), _, query, _, false)
      if query.resolved && needsSchemaAdjustment(d.name(), query, d.schema()) =>
      val projection = normalizeQueryColumns(query, d)
      if (projection != query) {
        a.copy(query = projection)
    ...
    case d@DeleteFromTable(table, condition) if d.childrenResolved =>
      val newTarget = table.transformUp { case StarLakeRelationV2(v2r) => v2r }
      val indices = newTarget.collect {
        case StarLakeTableRelationV2(tbl) => tbl
      }
      if (indices.isEmpty) {
        // Not a Star table at all, do not transform
        d
      } else if (indices.size == 1 && !indices.head.snapshotManagement.snapshot.isFirstCommit) {
        // It is a well-defined Star table with a schema
        StarDelete(newTarget, condition)
      } else {
        // Not a well-defined Star table
        throw StarLakeErrors.notAnStarLakeSourceException("DELETE", Some(d))
      }
    ...
    case u@UpdateTable(table, assignments, condition) if u.childrenResolved =>
      ...
      if (indices.isEmpty) {
        ...
        // It is a well-defined Star table with a schema
        StarUpdate(newTarget, cols, expressions, condition)
      } else {
        // Not a well-defined Star table
        throw StarLakeErrors.notAnStarLakeSourceException("UPDATE", Some(u))
      }
  ...
  /**
    ...
    * can check if the by-ordinal schema of the insert query matches our Star table.
    */
  private def normalizeQueryColumns(query: LogicalPlan, target: StarLakeTableV2): LogicalPlan = {
  ...
  /**
    * With StarLake, we ACCEPT_ANY_SCHEMA, meaning that Spark doesn't automatically adjust the schema
  ...
  private def needsSchemaAdjustment(
    ...
    if (output.length < schema.length) {
      throw StarLakeErrors.notEnoughColumnsInInsert(tableName, output.length, schema.length)
    }
  ...
  private def addCastsToStructs(
    ...
    if (source.length < target.length) {
      throw StarLakeErrors.notEnoughColumnsInInsert(
      ...
      case (StructField(name, nested: StructType, _, metadata), i) if i < target.length =>
        target(i).dataType match {
          ...
          case o =>
            ...
            throw StarLakeErrors.cannotInsertIntoColumn(tableName, field, targetName, o.simpleString)
        }
...
/** Matchers for dealing with a Star table. */
object StarLakeRelation {
  def unapply(plan: LogicalPlan): Option[LogicalRelation] = plan match {
    case dsv2@DataSourceV2Relation(d: StarLakeTableV2, _, _, _, _) => Some(fromV2Relation(d, dsv2))
    case lr@StarLakeTable(_) => Some(lr)
    case _ => None
  }

  def fromV2Relation(d: StarLakeTableV2, v2Relation: DataSourceV2Relation): LogicalRelation = {
    val relation = d.toBaseRelation
    LogicalRelation(relation, v2Relation.output, d.catalogTable, isStreaming = false)
  }
}

object StarLakeRelationV2 {
  def unapply(plan: LogicalPlan): Option[DataSourceV2Relation] = plan match {
    case dsv2@DataSourceV2Relation(t: StarLakeTableV2, _, _, _, _) => Some(dsv2)
    case _ => None
  }
