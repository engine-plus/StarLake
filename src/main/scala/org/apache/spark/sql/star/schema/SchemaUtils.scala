...
object SchemaUtils {
  // We use case insensitive resolution while writing into StarTable
    ...
    def generateSelectExpr(sf: StructField, nameStack: Seq[String]): Column = sf.dataType match {
      ...
      case a: ArrayType if typeExistsRecursively(a)(_.isInstanceOf[NullType]) =>
        ...
        throw new AnalysisException(
          s"Found nested NullType in column $colName which is of ArrayType. StarTable doesn't " +
            "support writing NullType in complex types.")
      case m: MapType if typeExistsRecursively(m)(_.isInstanceOf[NullType]) =>
        val colName = UnresolvedAttribute.apply(nameStack :+ sf.name).name
        throw new AnalysisException(
          s"Found nested NullType in column $colName which is of MapType. StarTable doesn't " +
            "support writing NullType in complex types.")
      case _ =>
        val colName = UnresolvedAttribute.apply(nameStack :+ sf.name).name
  ...
  /**
    * As the Star snapshots update, the schema may change as well. This method defines whether the
    * new schema of a Star table can be used with a previously analyzed LogicalPlan. Our
  ...
  def isReadCompatible(existingSchema: StructType, readSchema: StructType): Boolean = {
    ...
    def isStructReadCompatible(existing: StructType, newtype: StructType): Boolean = {
      ...
      assert(existingFieldNames.size == existingSchema.length,
        "Star tables don't allow field names that only differ by case")
  ...
  /**
    * Check whether we can write to the Star table, which has `tableSchema`, using a query that has
  ...
  /**
    * Verifies that the column names are acceptable by Parquet and henceforth StarTable. Parquet doesn't
  ...
  def checkFieldNames(names: Seq[String]): Unit = {
    ...
    names.find(_.contains("\n")).foreach(col => throw StarLakeErrors.invalidColumnName(col))
  }
