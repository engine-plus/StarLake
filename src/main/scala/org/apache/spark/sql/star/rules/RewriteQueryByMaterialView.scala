...
import org.apache.spark.sql.star.{ConstructQueryInfo, MaterialViewUtils, RangeDetail, StarLakeUtils}
...
case class RewriteQueryByMaterialView(spark: SparkSession)
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = {
    val canRewrite = if (spark.conf.contains(StarLakeUtils.USE_MATERIAL_REWRITE)) {
      spark.conf.get(StarLakeUtils.USE_MATERIAL_REWRITE).toBoolean
    } else {
      true
    }

    if (canRewrite &&
      spark.sessionState.conf.getConf(StarLakeSQLConf.MATERIAL_QUERY_REWRITE_ENABLE)) {
...
//        val asInfo = queryInfo.columnAsInfo.filter(f => f._1.contains(".`"))
        ...
        val queryRelateTables = queryInfo.tableInfo.values
          .map(m => {
            val split = m.split("\\.")
            assert(split(0).equals(StarLakeSourceUtils.NAME))
            split(1).replace("`", "")
          })
          .toSet
...
//              if((queryInfo.aggregateInfo.isEmpty && viewInfo.aggregateInfo.nonEmpty) ||
//                (queryInfo.aggregateInfo.nonEmpty && viewInfo.aggregateInfo.isEmpty)){
//                canRewrite = false
//              }else if(queryInfo.aggregateInfo.nonEmpty && viewInfo.aggregateInfo.nonEmpty){
//                if (!queryInfo.aggregateInfo.equals(viewInfo.aggregateInfo)){
//                  canRewrite = false
//                }
//              }
...
//              if (queryInfo.joinInfo.nonEmpty || viewInfo.joinInfo.nonEmpty) {
//                if (queryInfo.joinInfo.length != viewInfo.joinInfo.length) {
//                  canRewrite = false
//                } else {
//                  queryInfo.joinInfo.foreach(info => {
//                    val candidateInfo = viewInfo.joinInfo
//                      //first, join type should match
//                      .filter(f => info.joinType.equals(f.joinType))
//                      //second, join table should match
//                      .filter(f => {
//                      info.joinType match {
//                        case "INNER" | "CROSS" =>
//                          (info.leftTable.equals(f.leftTable) && info.rightTable.equals(f.rightTable)) ||
//                            (info.leftTable.equals(f.rightTable) && info.rightTable.equals(f.leftTable))
//
...
//                      }
//                    })
//                      //third, join condition should match
//                      .filter(f => {
//                      val equalConditionMatch = info.equalCondition.equals(f.equalCondition)
//                      val otherConditionMatch = info.otherCondition.equals(f.otherCondition)
//                      equalConditionMatch && otherConditionMatch
//                    })
//
//                    //one and only one join info should match
...
//              }
            ...
            if (canRewrite) {
              ...
              while (canRewrite && iter.hasNext) {
                  ...
                  if (RangeDetail.matchEqual(queryRange, viewRange)) {
                    ...
                    if (canRewrite) {
                      if (queryRange.lower == null && viewRange.lower != null) {
                        ...
                        val compare = RangeDetail.transAndCompareRange(queryRange.lower, viewRange.lower, queryRange.dataType)
                      ...
                      if (queryRange.upper == null && viewRange.upper != null) {
                        ...
                        val compare = RangeDetail.transAndCompareRange(queryRange.upper, viewRange.upper, queryRange.dataType)
                        if (compare > 0) {
                          canRewrite = false
                ...
                } else if (!conditionColumnsExists(colName, viewOutputColumns, viewInfo.equalInfo)) {
            ...
            //check other condition match
            if (canRewrite) {
              //other condition in view should be a subset of query
              val otherDiff = viewInfo.otherInfo.diff(queryInfo.otherInfo)
              if (otherDiff.nonEmpty) {
                canRewrite = false
              }
            }
        ...
        if (viewsCanBeUsed.nonEmpty) {
          ...
          def matchViewOutputReference(name: String): AttributeReference = {
...
//            val qualifiedName = ConstructQueryInfo.replaceByTableInfo(queryInfo.tableInfo, name)
            ...
            val fieldWithFinalTable = ConstructQueryInfo.replaceByTableInfo(queryInfo.tableInfo, name)
...
//            val findFromView = chosenView.info.outputInfo.find(f => f._2.equals(formattedName))
            val findFromView = findColumnFromViewOutput(
              ...
              chosenView.info.equalInfo)
            val viewColumnName = findFromView.split("\\.").last
            outputRef(viewColumnName)
          }
          ...
          def findNewAttributeReference(expression: Expression): Expression = {
            expression match {
              ...
              case attr: AttributeReference =>
                matchViewOutputReference(attr.sql)
              case leaf: LeafExpression => leaf
              case as@Alias(child, name) =>
                as.copy(child = matchViewOutputReference(child.sql))(
                  exprId = as.exprId,
              ...
              case unsupport => throw StarLakeErrors.unsupportedLogicalPlanWhileRewriteQueryException(unsupport.sql)
            }
          }

          def checkAndReplaceRangeCondition(attr: AttributeReference): (Boolean, Expression) = {
            val columnWithFinalTable = ConstructQueryInfo.replaceByTableInfo(queryInfo.tableInfo, attr.sql)
            ...
            if (equalRangeColumns.contains(formattedName)) {
              ...
              (false, findNewAttributeReference(attr))
            }
          }
          ...
          newPlan = plan resolveOperators {
            ...
            //remove alias for star data source
            case sa@SubqueryAlias(ident, child@DataSourceV2Relation(table: StarLakeTableV2, _, _, _, _))
              if ident.toString()
                .startsWith(s"${CatalogManager.SESSION_CATALOG_NAME}.${StarLakeSourceUtils.NAME}") =>
...
//              assert(queryInfo.tableInfo.size < 2)
  ...
  def conditionColumnsExists(colName: String,
                             ...
                             equalInfo: Seq[Set[String]]): Boolean ={
    if (viewOutput.contains(colName)){
      true
    }else{
      val find = equalInfo.find(f => f.contains(colName))
      if (find.isDefined){
  ...
  def findColumnFromViewOutput(colName: String,
                               ...
                               equalInfo: Seq[Set[String]]): String ={
    var find = outputInfo.find(f => f._2.equals(colName))
    if (find.isDefined){
      find.get._1
    }else{
      //it should be found because of previous check
      val equalColsItr = equalInfo.find(f => f.contains(colName)).get.iterator
      var notFound = true
  ...
  def buildDataSourceV2Relation(viewName: String): (String, DataSourceV2Relation) = {
    ...
    val table = StarLakeTableV2(spark, new Path(tablePath))
