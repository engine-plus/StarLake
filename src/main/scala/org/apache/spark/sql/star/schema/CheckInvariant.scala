...
import org.apache.spark.sql.star.schema.Invariants.{ArbitraryExpression, NotNull}
import org.apache.spark.sql.types.{DataType, NullType}

/** An expression that validates a specific invariant on a column, before writing into star table. */
case class CheckInvariant(child: Expression,
  ...
  private def generateNotNullCode(ctx: CodegenContext): Block = {
    ...
    code"""${childGen.code}
          |
       |if (${childGen.isNull}) {
          |  throw org.apache.spark.sql.star.schema.InvariantViolationException.apply(
          |    $invariantField, "");
          |}
     """.stripMargin
  }
