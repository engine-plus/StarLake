...
//    def getJoinTableName(source: LogicalPlan): String = {
//      source match {
//        case SubqueryAlias(ident, child) => ident.toString()
//        case DataSourceV2Relation(table: StarLakeTableV2, _,_,_,_) => table.name()
...
//          //unsupport other scene
//        case o => throw StarLakeErrors.unsupportedLogicalPlanWhileRewriteQueryException(o.toString())
//      }
//    }
        ...
        if(underAggregate){
          filters.foreach {
            case EqualTo(left, right) =>
              constructInfo.addEqualInfo(left.sql, right.sql)
...
//
//        val leftTable = getJoinTableName(left)
//        val rightTable = getJoinTableName(right)
//        parseJoinInfo.setJoinInfo(leftTable, rightTable, joinType.sql)
//
...
//        parseMaterialInfo(left, constructInfo)
//        parseMaterialInfo(right, constructInfo)
        ...
        if(condition.isDefined){
          if(allowJoinBelow){
            ...
            if(underAggregate){
              ...
              splitConjunctivePredicates(condition.get).foreach {
                case EqualTo(l, r) =>
                  constructInfo.addEqualInfo(l.sql, r.sql)
...
//        throw StarLakeErrors.unsupportedLogicalPlanWhileRewriteQueryException("havingCondition")
      ...
      case Aggregate(groupingExpressions, aggregateExpressions, child) =>
        ...
        //set alias info
        aggregateExpressions.foreach({
//          case as: Alias => constructInfo.addColumnAsInfo(as.qualifiedName, as.child.sql)
          case as: Alias => constructInfo.addColumnAsInfo("`" + as.name + "`", as.child.sql)
          case _ =>
        })
...
//      case sa @ SubqueryAlias(ident, SubqueryAlias(identifier, _)) =>
//        constructInfo.addTableInfo(ident.toString(), identifier.toString())
//        parseMaterialInfo(sa.child, constructInfo, underAggregate)
//
//      case SubqueryAlias(ident, child @ DataSourceV2Relation(table: StarLakeTableV2, _,_,_,_)) =>
//        constructInfo.addTableInfo(ident.toString(), table.name())
//        parseMaterialInfo(child, constructInfo, underAggregate)


      case SubqueryAlias(ident, child) =>
//        constructInfo.addTableInfo(ident.toString(), findSubAliasTable(child))
  ...
  private def parseCondition(condition: Expression, constructInfo: ConstructQueryInfo): Unit = {
    condition match {
      ...
      case e @ EqualNullSafe(left: AttributeReference, right: AttributeReference) =>
        constructInfo.addEqualInfo(left.sql, right.sql)
        constructInfo.addOtherInfo(e.sql)
...
//  private def findSubAliasTable(plan: LogicalPlan): String = {
//    plan match {
//      case DataSourceV2Relation(table, _, _, _, _) =>
//        assert(table.isInstanceOf[StarLakeTableV2])
//        table.name()
//      case other =>
//        //it should has only one child
//        assert(other.children.length == 1)
//        findSubAliasTable(other.children.head)
//    }
//  }
...
//  private def jointQualifierAndName(attr: AttributeReference): String ={
//    if(attr.qualifier.isEmpty){
//      attr.name
//    }else{
//      attr.qualifier.mkString(".") + "." + attr.name
//    }
//  }

//  private def jointQualifierAndName(qualifier: Seq[String], name: String): String ={
//    if(qualifier.isEmpty){
//      name
//    }else{
//      qualifier.mkString(".") + "." + name
//    }
//  }
