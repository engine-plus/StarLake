...
class MergeOperatorSuite extends QueryTest
  ...
  test("perform merge operator to field not in star table should failed") {
    new MergeOpString().register(spark, "stringOp")
    new MergeOpInt().register(spark, "intOp")
    ...
    assert(e1.getMessage().contains("is not in StarTable, you can't perform merge operator on it"))
    ...
    assert(e2.getMessage().contains("is not in StarTable, you can't perform merge operator on it"))
  }
