...
class ShuffleJoinSuite extends Logging {
  def run(): Unit = {
    ...
    try {
      ...
      StarTable.forPath(table_name1).upsert(allData1.select("range", "hash", "stu"))
      StarTable.forPath(table_name2).upsert(allData2.select("range", "hash", "stu"))
      ...
      StarTable.forPath(table_name1).toDF.persist().createOrReplaceTempView("e1")
      StarTable.forPath(table_name2).toDF.persist().createOrReplaceTempView("e2")
      ...
      StarTable.forPath(table_name2).dropTable()
      StarTable.forPath(table_name1).dropTable()
