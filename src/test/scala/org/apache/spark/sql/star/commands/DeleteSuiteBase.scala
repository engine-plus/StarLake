...
abstract class DeleteSuiteBase extends QueryTest
  ...
  protected def checkDelete(condition: Option[String],
    ...
    executeDelete(target = tableName.getOrElse(s"star.`$tempPath`"), where = condition.orNull)
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(s"basic case - delete from a Star table by path - Partition=$isPartitioned") {
      withTable("starTable") {
        val partitions = if (isPartitioned) "key" :: Nil else Nil
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(s"basic case with hash partition - delete from a Star table by path - rangePartition=$isPartitioned") {
      withTable("starTable") {
        val partitions = if (isPartitioned) "range" :: Nil else Nil
  ...
  test(s"basic case with hash partition - delete from a Star table by path") {
    withTable("starTable") {
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(s"basic case - delete from a Star table by name - Partition=$isPartitioned") {
  ...
  test("Negative case - non-deterministic condition") {
    ...
    val e = intercept[AnalysisException] {
      executeDelete(target = s"star.`$tempPath`", where = "rand() > 0.5")
    }.getMessage
    assert(e.contains("nondeterministic expressions are only allowed in"))
  }

  test("delete cached table by name") {
    withTable("cached_star_table") {
      Seq((2, 2), (1, 4)).toDF("key", "value")
        .write.format("star").saveAsTable("cached_star_table")

      spark.table("cached_star_table").cache()
      spark.table("cached_star_table").collect()
      executeDelete(target = "cached_star_table", where = "key = 2")
      checkAnswer(spark.table("cached_star_table"), Row(1, 4) :: Nil)
    }
  }
  ...
  test("do not support subquery test") {
    ...
    val e0 = intercept[AnalysisException] {
      executeDelete(target = s"star.`$tempPath`", "key < (SELECT max(c) FROM source)")
    }.getMessage
    assert(e0.contains("Subqueries are not supported"))

    // subquery with EXISTS
    val e1 = intercept[AnalysisException] {
      executeDelete(target = s"star.`$tempPath`", "EXISTS (SELECT max(c) FROM source)")
    }.getMessage
    assert(e1.contains("Subqueries are not supported"))

    // subquery with NOT EXISTS
    val e2 = intercept[AnalysisException] {
      executeDelete(target = s"star.`$tempPath`", "NOT EXISTS (SELECT max(c) FROM source)")
    }.getMessage
    assert(e2.contains("Subqueries are not supported"))

    // subquery with IN
    val e3 = intercept[AnalysisException] {
      executeDelete(target = s"star.`$tempPath`", "key IN (SELECT max(c) FROM source)")
    }.getMessage
    assert(e3.contains("Subqueries are not supported"))

    // subquery with NOT IN
    val e4 = intercept[AnalysisException] {
      executeDelete(target = s"star.`$tempPath`", "key NOT IN (SELECT max(c) FROM source)")
    }.getMessage
