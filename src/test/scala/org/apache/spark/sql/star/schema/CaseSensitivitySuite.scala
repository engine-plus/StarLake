...
class CaseSensitivitySuite extends QueryTest
  with SharedSparkSession with SQLTestUtils with StarLakeSQLCommandTest {
  ...
  test("set range partition columns with option - rangePartitions") {
    withTempDir { tempDir =>
      ...
      Seq((1, "a"), (2, "b")).toDF("Key", "val").write
        ...
        .format("star").mode("append").save(path)

      checkAnswer(
        spark.read.format("star").load(path).select("Key", "val"),
        Row(1, "a") :: Row(2, "b") :: Nil
      )
  ...
  test("set hash partition columns") {
    withTempDir { tempDir =>
      ...
      checkAnswer(
        spark.read.format("star").load(path).select("key", "val", "hash"),
        Row(3, "d", "3") :: Nil
      )
...
//  }
  ...
  testWithCaseSensitivity("two fields with same name") {
    withTempDir { tempDir =>
      intercept[AnalysisException] {
        ...
        //        sql(query).write.partitionBy("foo").format("star").save(tempDir.getAbsolutePath)
        sql(query).write.option("rangePartitions", "foo").format("star").save(tempDir.getAbsolutePath)
      }
    }
  }
  ...
  testWithCaseSensitivity("schema merging is case insenstive but preserves original case") {
    withTempDir { tempDir =>
      ...
      sql(query1).write.format("star").save(tempDir.getAbsolutePath)

      val query2 = "SELECT id + 1 as Foo, id as bar FROM RANGE(1)" // notice how 'F' is capitalized
      sql(query2).write.format("star").mode("append").save(tempDir.getAbsolutePath)

      val query3 = "SELECT id as bAr, id + 2 as Foo FROM RANGE(1)" // changed order as well
      sql(query3).write.format("star").mode("append").save(tempDir.getAbsolutePath)

      val df = spark.read.format("star").load(tempDir.getAbsolutePath)
      checkAnswer(
        df.select("foo", "bar"),
