...
class StarLakePostHocAnalysisSuite extends QueryTest with SharedSparkSession with StarLakeSQLCommandTest {
  ...
  Seq("intersect", "intersectAll").foreach(op => {
    test(s"$op on hash keys without shuffle and sort - same table") {
      withTempDir(dir => {
        ...
        val tableDF = StarTable.forPath(tableName).toDF.persist()
    ...
    test(s"$op on hash keys without shuffle and sort - different table") {
      withTempDir(dir1 => {
        withTempDir(dir2 => {
          ...
          val table1 = StarTable.forPath(tableName1).toDF.persist()
          val table2 = StarTable.forPath(tableName2).toDF.persist()
  ...
  test("star table has no duplicate hash data") {
    withTempDir(dir => {
      val tableName = dir.getCanonicalPath
      ...
      val table = StarTable.forPath(tableName).toDF.persist()
