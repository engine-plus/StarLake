  ...
  def commitTest(commitType: String, changeSchema: Boolean): Unit = {
    test(s"$commitType commit, change schema: $changeSchema") {
      withTempDir(tmpDir => {
        ...
        MetaCommit.doMetaCommit(metaInfo, changeSchema, None)
        ...
        for (i <- 0 until taskNum) {
          pool.execute(new Runnable {
            override def run(): Unit = {
              MetaCommit.doMetaCommit(arrMetaInfo(i), false, None)
            }
          })
...
class MetaCommitSuite extends MetaCommitSuiteBase {
  ...
  test("Committing state will roll back when timeout") {
    withTempDir(tmpDir => {
      ...
      val metaInfo2 = MetaInfo(
        ...
        CommitType("simple"))
      MetaCommit.doMetaCommit(metaInfo2, false, None)
  ...
  test("check files conflict - files change while commit will throw MetaRerunException") {
    withTempDir(tmpDir => {
      ...
      MetaCommit.doMetaCommit(metaInfo1, false, None)

      val e = intercept[MetaRerunException](MetaCommit.doMetaCommit(metaInfo2, false, None))
  ...
  test("take schema lock concurrently") {
    withTempDir(tmpDir => {
      ...
      MetaCommit.doMetaCommit(metaInfo2, true, None)
      ...
      val e = intercept[AnalysisException] {
        MetaCommit.doMetaCommit(metaInfo3, true, None)
      }
